...
1. **Token 预检优先**：每次回答/调用前先执行 `kweaver auth status`；token 不可用时先刷新后再继续。  
2. **意图优先**：须先完成步骤二内「**独立安全拦截 · 写操作**」（与 [SKILL.md](../SKILL.md) §1) 意图识别 一致，**最先**执行），再判定「**查询权限**」还是「**申请权限**」，再构造请求；未完成安全拦截、意图识别与参数校验前 **不得** 调用接口。  
3. **写操作与安全拦截**：业务数据删改、裸 DML/DDL、停机与破坏性命令须硬拦截并终止；权限申请、行列规则治理写操作、只读发现检索经「治理写操作例外」放行；细则以 [SKILL.md](../SKILL.md) 步骤二为准。  
4. **同轮并存**：若同轮同时含申请与查询，须澄清顺序；**默认先查后申**。  
5. **失败即停**：任一步失败须停止并返回真实错误，不得假称成功。  
6. **进度硬约束**：每完成总入口或分支内任一步，须立即输出「任务进度清单」；当前步未输出进度前不得进入下一步；失败即停，不得将后续步骤标为已完成；分支结束须标注 **流程完成**。清单格式与步骤定义以 [SKILL.md](../SKILL.md) 为准。

## 1.1 接口统一鉴权重试（防死循环）

1. **适用范围**：本技能内所有 HTTP 接口（包含申请、查询、资源发现、申请人发现、数字员工、行列规则 CRUD）。  
2. **触发条件**：首次调用出现 `401`、`403`、`Unauthorized`、`token expired` 等鉴权相关错误。  
3. **执行动作**：先刷新 token（优先 `refresh_token` 自动换发；失败则重新登录），再按原参数重试同一请求。  
4. **重试上限**：每个接口请求最多 **1 次重试**（总尝试次数最多 2 次）。  
5. **终止条件**：重试后仍失败时立即返回原始错误，禁止递归/循环继续重试。
...
1. **Bearer**：调用前须具备有效 `access_token`，并以 `Authorization: Bearer <access_token>` 发送；获取路径（`kweaver auth login`、`kweaver call` 自动注入、curl/浏览器/CI）见 [references/authentication.md](../references/authentication.md)。
2. **刷新策略**：优先复用 `~/.kweaver/` 中 `refresh_token` 自动换发；若换发失败或无 refresh_token，执行 `kweaver auth login <url>` 重新登录。  
3. **静态 Token 限制**：`KWEAVER_TOKEN` 静态凭据不支持自动 refresh，过期后必须替换为新 token。
